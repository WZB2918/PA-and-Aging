...
data monitor;
  merge work.pam_perminute(in=in_pam) work.demo_d(in=in_demog keep=seqn ridageyr);
  by seqn;
  if in_pam;
...
run;
data pam_perperson;
  merge pam_all(in=in_pam keep=seqn valid_person valdays) work.demo_d(in=in_demog) valid_days;* changed by kwd;
  by seqn;
  if in_pam;
  if not in_demog then put 'error: not in demog' seqn=;
